...
**Output format for reviews:** When reviewing code, output each finding as: `[ID] [SEVERITY] [WCAG] - Description. Fix: (fix text).` List findings in severity order: CRITICAL first, then IMPORTANT, then SUGGESTION. If no issues are found, state: `No accessibility issues detected.`

**Detection interpretation:** Detection fields describe the code pattern to look for when reviewing files. They are expressed in regex-like notation as shorthand for the pattern intent, not as executable expressions. Interpret each detection field conceptually.

**Deduplication rule:** When the same accessibility failure matches both a general pattern (K1-K7) and a framework-specific pattern (RX, NG, VU), report only the framework-specific pattern with its framework-appropriate fix. Do not report both.
...
- **SUGGESTION** — Improves usability for assistive technology or documents optional AAA-only enhancements beyond the AA target. Plan for a future iteration.
...
**Target**: WCAG 2.2 AA is a superset of 2.1 AA and 2.0 AA. Meeting WCAG 2.2 AA satisfies current legal baselines, but EAA legal minimum maps to WCAG 2.1 AA via EN 301 549.
...
- **Severity**: SUGGESTION
- **Detection**: More than one `<h1>` element present in the document
- **WCAG**: Best practice (supports 1.3.1)

Flag all occurrences. Multiple `<h1>` elements are not a strict WCAG violation but are reported as a best-practice suggestion because they can make heading navigation less clear.
...
If `aria-describedby` is present but the referenced ID does not exist in the same file or component scope, flag as IMPORTANT with message: "aria-describedby references ID `[id]` which is not found in this file. Verify the element exists in the rendered DOM at runtime. Missing referenced IDs cause screen readers to silently ignore the association."
...
- **Severity**: IMPORTANT
- **Detection**: Puzzle/image CAPTCHAs without accessible alternative or fallback path
- **WCAG**: 3.3.8 (AA)

Use reCAPTCHA v3 (invisible), hCaptcha accessibility mode, or alternative authentication.

### F5b: Password Field Blocking Paste or Autofill

- **Severity**: IMPORTANT
- **Detection**: `type="password"` with `autocomplete="off"` or JavaScript that prevents paste events
- **WCAG**: 3.3.8 (AA), 1.3.5 (AA)
...
- **Severity**: IMPORTANT
- **Detection**: `font-size:\s*[0-9.]+px` on any selector other than `:root` or `body`
- **WCAG**: 1.4.4 (AA)

Always flag pixel font sizes on component or element selectors. `:root { font-size: 16px }` and `body { font-size: 16px }` are permitted as base definitions. Use `rem` or `em` for content fonts.
...
### D5: Informational SVG Without Accessible Name

- **Severity**: CRITICAL
- **Detection**: `<svg` without `role="img"` and without `aria-labelledby` or `aria-label`, where the SVG is not inside a `<button>` or `<a>` with its own accessible name
- **WCAG**: 1.1.1 (A)

Add `role="img"` and a `<title>` element as the first child, optionally with `<desc>`. Reference the title via `aria-labelledby`.

```html
<svg role="img" aria-labelledby="chart-title">
  <title id="chart-title">Monthly revenue, Jan-Dec 2024</title>
  ...
</svg>
```
...
For Next.js v13+ projects: do not flag route changes as missing a live region by default. Instead, verify that every page has a unique `<title>` and flag pages missing a unique title under this rule.
